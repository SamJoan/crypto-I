...
import string, math, itertools, sys
from pprint import pprint

printable = string.printable + '\x00'
...
def bytes_pretty(string):
    for c in string:
        o = ord(c)
        print("{0:b}".format(o).rjust(8, "0")),
...
def bytes_pretty_three(a, b, c):
    ...
    print "=============================="
    print bytes_pretty(c)
    print

def hex_to_string(hexa):
    new_s = ""
    l = len(hexa) / 2
    try:
        for x in range(l):
...
if __name__ == "__main__":
    valid = True
    poss = "The secret message is: When using a stream cipher, never use the key more than once"
    #poss = "You don't"
    ...
    for e in encrypted_strings:
        ...
        print xor_poss
        if not all_in_set(xor_poss, correct):
